...
export function traceRail(dimensionLocation, distance, enter, traceOption) {
    let dimension = dimensionLocation.dimension;
    let location = { x: dimensionLocation.x, y: dimensionLocation.y, z: dimensionLocation.z };
    //get block location
    let blockLocation = toBlockLocation(location);
    let currentBlock = dimension.getBlock(blockLocation);
    ...
    if (railDirection[state][enter].ascending == Direction.Up)
